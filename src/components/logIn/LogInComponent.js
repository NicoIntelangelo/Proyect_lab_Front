...
const LogInComponent = ({ toggleRegisterLogin, authentication }) => {
  ...
  const authenticateUser = async () => {
    try {
      ...
      const response = await fetch("https://localhost:7254/auth/authenticate", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        const roleResponse = await fetch("https://localhost:7254/user/role", {
          method: "GET",
          headers: {
            "content-type": "application/json",
  ...
  return (
    <div
      className={
        theme === "dark" ? "l-container l-container-dark dark" : "l-container"
      }
    >
      <div>
        {showAlert && (
